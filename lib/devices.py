...
class InputDevice (object):
	...
	def receive (self, event):
		...
		if event.type == const.EV_KEY and self._inMap(key):
			...
			#self._changeMode (key) # ERROR - this seems to not work
			pass
	...
	def _relayCombo (self, combo, value=0):
		...
		# Alternative mode set
		if len(combo)==1 and combo[0].string.startswith('LED_'):
			self._changeMode(mapping.Key(combo[0].string, value, value))
